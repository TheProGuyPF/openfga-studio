...
/**
 * Migrates assessment-template rows from a CSV into structural OpenFGA tuples:
 * `parent_institution` on `assessment_template` (see assessments.fga).
 *
 * On `assessment_template`, `parent_institution: [institution]` is the only
 * structural relation we can derive from this CSV; all other relations
 * (reader/editor/deleter/sharer/creator) are direct user|role assignments and
 * are not present in the template export.
 *
 * Rows with `deleted=true` are skipped (soft-deleted templates should not
 * receive fresh permissions). Rows missing `id` or `institution_id` are skipped.
 *
 * Writes set `on_duplicate: ignore` so tuples that already exist are skipped
 * without failing the batch.
 *
 * CSV must be RFC 4180 (quoted fields may contain commas and newlines);
 * do not use naive comma-splitting exports. The upstream `name` column is a
 * JSON blob that may contain commas, which csv-parse handles correctly here.
 *
 ...
 *
 * Usage:
 *   node scripts/migrate-assessment-template-tuples.mjs --csv <path> --model-id <id> [--dry-run] [--batch-size <n>]
 *
 * Examples:
 *   node scripts/migrate-assessment-template-tuples.mjs --csv scripts/data/mx-templates-npe.csv --dry-run
 *   node scripts/migrate-assessment-template-tuples.mjs --csv scripts/data/mx-templates-npe.csv --model-id <authorization-model-id>
 */
...
/** Parse the `deleted` CSV cell loosely: "true"/"TRUE"/"True" → true, else → false. */
function isDeleted(value) {
  return typeof value === "string" && value.trim().toLowerCase() === "true";
}

// ---------------------------------------------------------------------------
// Map a CSV row to one OpenFGA tuple (or [] if skipped)
// ---------------------------------------------------------------------------

function rowToTemplateTuples(row, counters) {
  if (isDeleted(row.deleted)) {
    counters.skippedDeleted++;
    return [];
  }
  ...
  if (!id || !institutionId) {
    counters.skippedMissingIds++;
    return [];
  }

  const userInstitution = `institution:${institutionId}`;
  const objectTemplate = `assessment_template:${id}`;
  if (
    !isValidOpenFgaTupleSegment(userInstitution) ||
    !isValidOpenFgaTupleSegment(objectTemplate)
  ) {
    counters.skippedInvalidFormat++;
    return [];
  }
...
async function main() {
  ...
  const tuples = rows.flatMap((row) => rowToTemplateTuples(row, counters));

  console.log(`  Skipped (deleted=true): ${counters.skippedDeleted}`);
  console.log(
    `  Skipped (missing id or institution_id): ${counters.skippedMissingIds}`
  );
  if (counters.skippedInvalidFormat > 0) {
    console.log(
      `  Skipped (ids fail OpenFGA tuple string rules): ${counters.skippedInvalidFormat}`
    );
  }
  console.log(`  parent_institution tuples  : ${tuples.length}`);
  ...
  if (failures.length > 0) {
    ...
    const failPath = path.resolve("migration-assessment-template-failures.json");
