...
/**
 * Migrates user–institution membership rows from a CSV into OpenFGA tuples
 * (`admin` / `member` on `institution`). For other tuple types, use a different script.
 * Writes set `on_duplicate: ignore` so tuples that already exist are skipped without failing the batch.
 *
 * Usage:
 *   node scripts/migrate-user-tuples.mjs --csv <path> --model-id <id> [--dry-run] [--batch-size <n>]
 *
 * Examples:
 *   # Preview the tuples without writing anything
 *   node scripts/migrate-user-tuples.mjs --csv path/to/your-data.csv --dry-run
 *
 *   # Execute the migration
 *   node scripts/migrate-user-tuples.mjs --csv path/to/your-data.csv --model-id <your-authorization-model-id>
 *
 *   # With a custom batch size
 *   node scripts/migrate-user-tuples.mjs --csv path/to/your-data.csv --model-id <id> --batch-size 50
 */
...
async function main() {
  ...
  if (failures.length > 0) {
    ...
    const failPath = path.resolve("migration-user-tuples-failures.json");
